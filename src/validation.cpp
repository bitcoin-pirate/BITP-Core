...
#if defined(NDEBUG)
# error "BitcoinPirate cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "BitcoinPirate Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s BITPS additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
   int halvings = nHeight / consensusParams.nSubsidyHalvingInterval;
    ...
    if (nHeight == 1) return 8400000000 * COIN; // for genesis block reward
...
void ThreadScriptCheck() {
    RenameThread("bitcoinpirate-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // BITP: We don't currently (re-)invoke ContextualCheckBlock() or
 ...
 *  BITP: This function is not currently invoked by ConnectBlock(), so we
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // BITP: CheckBlockHeader is called by CheckBlock
    if (!ContextualCheckBlockHeader(block, state, chainparams, pindexPrev, GetAdjustedTime()))
